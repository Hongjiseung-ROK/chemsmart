...
transition-state, excited-state, relaxed coordinate scans, constrained optimisations, intrinsic reaction
coordinates, and serial producer-to-consumer DAGs. ``neb`` still requires qualification on the selected target
...
which value. To relax a structure at a value it does not yet have, set that coordinate on the geometry first; to
sample a range of values, use ``scan``. A completed constrained optimisation reports which coordinates were held,
by which atoms and at what value, each kind in its own unit, measured in the structure ORCA returned.
