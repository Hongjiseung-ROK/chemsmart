...
from chemsmart.jobs.pyscf.settings import (
    ...
    PYSCF_STABILITY_EIGENVALUE_UNIT,
...
#: v7 adds ``scf_stability``: whether the driver asked PySCF whether the
#: converged reference is a minimum in orbital-rotation space, and the
...
#: v10 adds the decomposition of the total energy the program itself
#: computed: ``solvation_electrostatic_energy`` and, under SMD,
#: ``solvation_nonelectrostatic_energy``, and ``dispersion_energy`` -- the
#: terms PySCF adds into ``e_tot`` and records in its own ``scf_summary``
#: at the converged density of the final geometry.  It adds no
#: applied-spec field, because every setting that produces a term is
#: already in the vocabulary (``solvent_model``, ``solvent_id``,
#: ``solvent_eps``, ``dispersion``), so its digest vocabulary *is* v9's,
#: exactly as v9's was v8's.  The version moves because under v10 a
...
#: The v6 vocabulary, frozen: the surface identity and the Hessian
#: derivative controls, over which every archived v6 digest was computed.
APPLIED_SPEC_FIELDS_V6 = APPLIED_SPEC_FIELDS_V5 + (
    "surface",
    "hessian_derivative",
    "fd_step_angstrom",
)
...
#: The v9 vocabulary, frozen.  Identical to v8's by construction: the
...
RESULT_UNITS = {
    ...
    "dipole_moment": "Debye",
    # The decomposition of the total (contract v10): the terms PySCF adds
    # into ``e_tot`` and records in ``scf_summary`` at the converged
    # density.  Hartree, as every other energy this driver writes.
...
class PySCFScriptWriter:
    ...
    def build_config(self):
        ...
        config = {
            ...
            "scf_stability": bool(
                getattr(settings, "scf_stability", False) is True
            ),
...
#: The parts of its own total PySCF records, and the dataset each is
#: written under.  ``e_solvent`` is the continuum's polarisation energy
#: (0.5 q.V at the converged density, the term ORCA prints as "CPCM
#: Dielectric"); ``e_cds`` is SMD's cavitation-dispersion-solvent-structure
#: term (ORCA's "SMD CDS (Gcds)"), which the PCM family does not have; and
#: ``dispersion`` is the semi-empirical correction.  Each is already inside
#: ``e_tot``: this records what the program added, it does not add anything.
ENERGY_DECOMPOSITION_TERMS = (
...
def _summary_scalar(summary, key):
    """One finite scalar from PySCF's ``scf_summary``, or None.

    None is the only other answer: a term the program did not record is
    absent, never zero, because zero reads as "it contributed nothing"
    and a continuum that was never applied contributed nothing in a
    different sense than one that contributed nothing.
    """
    if not isinstance(summary, dict) or key not in summary:
        return None
    value = _to_host_array(summary[key])
    try:
        values = np.asarray(value, dtype=float).reshape(-1)
    except (TypeError, ValueError):
        return None
    if values.size != 1 or not bool(np.isfinite(values[0])):
        return None
    return float(values[0])


def _energy_decomposition(mf):
    """The terms PySCF put into the total it reports, in Hartree.

    Read off the program's own ``scf_summary`` after the final SCF, so
    every term belongs to the density and the geometry every other
    property of this artifact belongs to.  Nothing here is recomputed:
    an energy the host derived itself would be a second authority on a
    number the program already owns.
    """
    summary = getattr(mf, "scf_summary", None)
    terms = {}
    for name, key in ENERGY_DECOMPOSITION_TERMS:
        value = _summary_scalar(summary, key)
...
def _stability_family(mf):
    """The reference family of the object PySCF actually built, or None.

    Resolved by isinstance rather than from the requested settings, so a
    runtime reference substitution cannot be described with the space
    names of the reference that was asked for.  The most specific class
    is tested first: ROKS derives from ROHF, ROHF from RHF, and UKS from
    UHF.  A density-fitting or solvent wrapper subclasses the reference
    it wraps, so it resolves to that reference.
    """
    from pyscf import dft, scf
...
def _scf_stability(mf):
    ...
    The two questions are asked in two separate calls on purpose.
    ``pyscf.scf.stability.rohf_stability`` runs the internal Davidson and
    then raises ``NotImplementedError`` from ``rohf_external``, so one
    combined call throws away an answer it already computed.

    Each answer is recorded with the orbital-rotation space it is about,
    ...
    record = {
        "status": "ok",
        # The analysis is about the mean-field reference, never about a
        # correlated or excited surface built on top of it.
        "applies_to": "reference",
        "reference_family": family,
        "reference_class": _class_name(mf),
        # The orbitals the analysis ran on: an unconverged SCF still
        # answers, about orbitals that are not stationary.
        "scf_converged": bool(getattr(mf, "converged", False)),
        "source": "pyscf.scf.hf.SCF.stability",
        "analyses": {},
        "not_determined": {},
    }
    if family is None:
    ...
    for question in ("internal", "external"):
        ...
        try:
            ...
            if value is None:
                ...
                entry["stable"] = bool(value)
                external_answered = (
                    external_answered or question == "external"
                )
...
def main():
    ...
    try:
        ...
        # What the total is made of (contract v10).  A solvated energy read
        # beside a gas-phase one is a solvation free energy only if the host
        # can say which is which and what the continuum contributed; the
        # program computed both terms and threw them away at the boundary.
        try:
            decomposition = _energy_decomposition(mf)
            results.update(decomposition)
            solvated = CONFIG.get("solvent_call") is not None
            electrostatic = "solvation_electrostatic_energy" in decomposition
            if not solvated:
                status["properties"]["solvation_energy"] = {
                    "status": "not_applicable",
                    "reason": "no continuum solvent model was applied",
                }
            elif not electrostatic:
                status["properties"]["solvation_energy"] = {
                    "status": "unavailable",
                    "failure": {
                        "type": "MissingSummaryTerm",
                        "message": (
                            "a continuum model was applied and PySCF "
                            "recorded no e_solvent in scf_summary"
                        ),
                    },
                }
            else:
                record = {
                    "status": "ok",
                    "source": "pyscf scf_summary",
                    "solvent_call": CONFIG.get("solvent_call"),
                }
                if "solvation_nonelectrostatic_energy" not in decomposition:
                    # The PCM family is electrostatics only.  The missing
                    # CDS term is the difference between the models, so it
                    # is stated as inapplicable rather than as a failure.
                    record["nonelectrostatic_term"] = "not_applicable"
                status["properties"]["solvation_energy"] = record
            if CONFIG.get("dispersion"):
                status["properties"]["dispersion_energy"] = (
                    {"status": "ok", "source": "pyscf scf_summary"}
                    if "dispersion_energy" in decomposition
                    else {
                        "status": "unavailable",
                        "failure": {
                            "type": "MissingSummaryTerm",
                            "message": (
                                "a dispersion correction was applied and "
                                "PySCF recorded none in scf_summary"
                            ),
                        },
                    }
                )
                ...
                status["properties"]["dispersion_energy"] = {
                    "status": "not_applicable",
                    "reason": "no dispersion correction was applied",
                }
        except Exception as exc:
            for name in ("solvation_energy", "dispersion_energy"):
        ...
        # Whether the converged reference is a minimum in orbital-rotation
        # space, or a saddle every number above it stands on. An
        # observation: no value of it changes a stage, a convergence flag
        # or a termination word.
        if CONFIG.get("scf_stability"):
            try:
                status["properties"]["scf_stability"] = _scf_stability(mf)
            except Exception as exc:
            ...
            status["properties"]["scf_stability"] = {
                ...
                "reason": (
                    "scf_stability was not requested; nothing here says "
...
# The space names are the settings module's declaration, substituted rather
# than retyped: a writer and a reader that each hold their own copy of one
# table are two tables that can disagree.
_SKELETON = _SKELETON.replace(
    "__CHEMSMART_STABILITY_SPACES__", repr(PYSCF_STABILITY_SPACES)
)
_SKELETON = _SKELETON.replace(
...
_SKELETON = _SKELETON.replace(
    "__CHEMSMART_STABILITY_EIGENVALUE_UNIT__",
    repr(PYSCF_STABILITY_EIGENVALUE_UNIT),
)
