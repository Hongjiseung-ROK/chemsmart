...
class GaussianJobSettings(MolecularJobSettings):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # A relaxed scan runs no frequency step: the writer has never
        # written one for it, and the reader declares nothing vibrational
        # for it.  The settings say so where they are built, because the
        # loader hands a scan the phase section's (or the shared default's)
        # ``freq: true`` and the preview compares that declaration with
        # the written route -- which made every Agent preview of a
        # Gaussian scan from an ordinary project red on ``freq``.
    ...
    def _get_dieze_tag(self):
        ...
        # for every modred route, so the route ran a frequency step a
        # project had declared off -- and the preview, which compares the
        # declaration with the written input, went red on it -- and set
        # ``self.freq = False`` for a scan, changing the settings object
        # by reading its route.  A scan still writes no frequency step.
        freq, numfreq = self.freq, self.numfreq
        if self.jobtype == "scan":
            freq = numfreq = False
        ...
        if explicit_frequency is not None:
            if numfreq and explicit_frequency.lower() != "freq=numer":
                raise ValueError(
