...
REFERENCE_TOPICS: tuple[ReferenceTopicV1, ...] = (
    ...
    ReferenceTopicV1(
        ...
        body=(
            ...
            "scientific judgement. An ORCA scan's validated minimum-energy "
            "sampled point may feed a downstream optimisation inside one "
            "approval through the declared producer edge; a Gaussian "
            "scan's points, and any other point of an ORCA scan, travel by "
            "an explicit scan-point binding whose consuming stage is a new "
    ...
    ReferenceTopicV1(
        ...
        body=(
            ...
            "level behind every input so the reviewer can tell. The host "
            "writes a functional literal in each program's own spelling and "
            "a result's functional selector reports the literal whose form "
            "that program says it applied, so a functional is compared by "
            "that value rather than by the word a project used. A correlated "
