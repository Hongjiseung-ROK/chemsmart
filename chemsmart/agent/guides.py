...
GUIDES: tuple[GuideV1, ...] = (
    ...
    GuideV1(
        ...
        body=(
            ...
            "coincide and the host checks it. A ts climbs to a saddle of its "
            "own surface from the seed you hand it, taking that surface's "
            "analytic Hessian there and recording the seed's spectrum and "
            "gradient, so what the search started from is visible. It claims "
            "no order where it lands: converged means the gradient is zero, "
            "and a hess node on the geometry it reached is what says which "
            "stationary point that is -- seeded at a minimum, a ts converges "
            "at once and returns the minimum. Its endpoint is what gives an "
            "irc a saddle of the surface the branch will walk. A hess is judged by the 20 "
            ...
            "small gap is a sensor fact, not a verdict; gas phase only. "
            "PCM/SMD give td "
            ...
            "Not available here: scans, a ccsd(t) "
            "optimisation, EOM or CASSCF, GPU execution, double hybrids, "
            "mixed basis/ECP, a Hessian for ROHF or an open-shell NLC "
            "functional; only the geometric optimiser is installed. A "
            "non-converged optimisation still "
    ...
    GuideV1(
        ...
        body=(
            ...
            "An ORCA IRC consumes the transition state's own geometry and "
            "analytic Hessian as role-distinct producer edges, one per "
            "direction; the path goes to an XYZ sidecar. A PySCF irc takes "
            "only the geometry and computes its own Hessian there, and its "
            "result carries the path, so its endpoint can feed a later "
            "node. Either way, whether the saddle connects two particular "
            "minima is an observation you make from the two branches, not "
            "a host-rendered claim. A "
